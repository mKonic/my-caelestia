...
Singleton {
    ...
    function dispatch(request: string): void {
        Hyprland.dispatch(root._toLua(request));
    }

    // Under a .lua Hyprland config, `hyprctl dispatch X` (which is what
    // Hyprland.dispatch ends up doing) is wrapped server-side as
    // `return hl.dispatch(X)` and parsed as Lua — legacy space-separated
    // hyprlang syntax always fails.  Translate the forms used across this
    // shell to a Lua *dispatcher object expression* (NOT pre-wrapped in
    // hl.dispatch — that would double-wrap and error). Pass anything we
    // don't recognise through verbatim.
    function _toLua(req: string): string {
        let m;

        // togglespecialworkspace [NAME]
        m = req.match(/^togglespecialworkspace\s*(.*)$/);
        if (m) {
            const name = m[1].trim() || "special";
            return `hl.dsp.workspace.toggle_special("${name}")`;
        }

        // workspace NAME (incl. r+1 / r-1 / e+0 / special:foo / etc.)
        m = req.match(/^workspace\s+(\S+)$/);
        if (m) return `hl.dsp.focus({workspace="${m[1]}"})`;

        // movetoworkspace WS,address:ADDR  (window-targeted move)
        m = req.match(/^movetoworkspace\s+(\S+?),address:(0x[0-9a-f]+)$/);
        if (m) return `hl.dsp.window.move({workspace="${m[1]}",address="${m[2]}"})`;

        // togglefloating address:ADDR
        m = req.match(/^togglefloating\s+address:(0x[0-9a-f]+)$/);
        if (m) return `hl.dsp.window.float({action="toggle",address="${m[1]}"})`;

        // pin address:ADDR
        m = req.match(/^pin\s+address:(0x[0-9a-f]+)$/);
        if (m) return `hl.dsp.window.pin({address="${m[1]}"})`;

        // killwindow address:ADDR
        m = req.match(/^killwindow\s+address:(0x[0-9a-f]+)$/);
        if (m) return `hl.dsp.window.close({address="${m[1]}"})`;
